...
#     ANNOTATIONS_PATH = "D:\cv\code\dataset\SSDD\Annotations_sub"  # xml文件所在文件夹
...
# Define number of clusters (anchors)
num_clusters = 9

# Extract bounding box widths and heights from VOC annotations
boxes = []
for root, _, files in os.walk('D:\zql\dataset\\root\BBox_SSDD\\voc_style\Annotations'):
    for file in files:
        tree = ET.parse(os.path.join(root, file))
        for obj in tree.findall('object'):
            bbox = obj.find('bndbox')
            boxes.append([int(bbox.find('xmax').text) - int(bbox.find('xmin').text),
                          int(bbox.find('ymax').text) - int(bbox.find('ymin').text)])
boxes = np.array(boxes)

# K-means clustering to determine anchor boxes
criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 10, 1.0)
centers = cv2.kmeans(boxes.astype('float32'), num_clusters, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)
